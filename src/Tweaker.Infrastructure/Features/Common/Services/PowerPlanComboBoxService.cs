        ...
        foreach (var predefinedPlan in PowerPlanDefinitions.BuiltInPowerPlans)
        {
            ...
            if (PowerPlanDefinitions.IsTweakivoPowerPlanGuid(predefinedPlan.Guid))
            {
                var brandedCandidates = systemPlans.Where(sp =>
                    PowerPlanDefinitions.IsTweakivoPowerPlanGuid(sp.Guid) ||
                    PowerPlanDefinitions.IsTweakivoPowerPlanName(Utilities.PowerPlanHelper.CleanPlanName(sp.Name)))
                    .ToList();

                matchingSystemPlan = brandedCandidates.FirstOrDefault(sp => sp.IsActive)
                    ?? brandedCandidates.FirstOrDefault(sp => PowerPlanDefinitions.IsTweakivoPowerPlanGuid(sp.Guid))
                    ?? brandedCandidates.FirstOrDefault();
                matchMethod = matchingSystemPlan != null && PowerPlanDefinitions.IsTweakivoPowerPlanGuid(matchingSystemPlan.Guid)
                    ? "canonical GUID"
