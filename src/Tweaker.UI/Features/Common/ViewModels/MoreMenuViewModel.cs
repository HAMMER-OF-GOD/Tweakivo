...
public partial class MoreMenuViewModel : ObservableObject, IDisposable
{
    ...
    private void InitializeVersionInfo()
    {
        try
        {
            ...
            VersionInfo = $"Tweakivo {versionInfo.Version}";
        }
        catch (Exception ex)
        {
    ...
    public string MenuTweakerLogs =>
        _localizationService.GetString("Menu_TweakerLogs") ?? "Tweakivo Logs";
    ...
    public string MenuTweakerScripts =>
        _localizationService.GetString("Menu_TweakerScripts") ?? "Tweakivo Scripts";

    public string MenuCloseTweaker =>
        _localizationService.GetString("Menu_CloseTweaker") ?? "Close Tweakivo";
    ...
    private async Task ReportBugAsync()
    {
        try
        {
            var issueUrl = Environment.GetEnvironmentVariable("TWEAKIVO_ISSUES_URL");
