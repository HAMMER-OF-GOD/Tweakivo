# PowerShell script that downloads the latest version of Tweakivo and runs the installer
...
function Get-FileFromWeb {
    ...
    try {
        ...
        do {
            ...
            $total += $count
    if ($fullSize -gt 0) { Show-Progress -TotalValue $fullSize -CurrentValue $total -ProgressText " Downloading Tweakivo" }
        } while ($count -gt 0)
    }
    finally {
        $reader.Close()
...
$downloadUrl = $env:TWEAKIVO_RELEASE_DOWNLOAD_URL
if ([string]::IsNullOrWhiteSpace($downloadUrl)) { throw "TWEAKIVO_RELEASE_DOWNLOAD_URL is not configured." }
...
Write-Host "          Tweakivo Installer" -ForegroundColor Cyan
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host ""
Write-Host "Downloading Tweakivo ($installType)..." -ForegroundColor Cyan
    ...
    Write-Host "Installing Tweakivo..." -ForegroundColor Cyan
    ...
    Write-Host "Launching Tweakivo..." -ForegroundColor Cyan
        ...
        Write-Host ""
    Write-Host "Extracting Tweakivo Portable to Desktop..." -ForegroundColor Cyan
