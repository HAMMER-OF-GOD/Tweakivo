# PowerShell script that downloads the latest beta (pre-release) version of Tweakivo and runs the installer
...
function Get-FileFromWeb {
    ...
    try {
        ...
        do {
            ...
            $total += $count
    if ($fullSize -gt 0) { Show-Progress -TotalValue $fullSize -CurrentValue $total -ProgressText " Downloading Tweakivo Beta" }
        } while ($count -gt 0)
    }
    finally {
        $reader.Close()
...
try {
Write-Host "Checking for latest Tweakivo beta release..." -ForegroundColor Cyan

    # Query GitHub API for the latest pre-release
    $releaseApiUrl = $env:TWEAKIVO_RELEASE_API_URL
    if ([string]::IsNullOrWhiteSpace($releaseApiUrl)) { throw "TWEAKIVO_RELEASE_API_URL is not configured." }
    $releases = Invoke-RestMethod -Uri $releaseApiUrl -Headers @{ "User-Agent" = "Tweakivo-Beta-Downloader" }
...
Write-Host "        Tweakivo Beta Installer" -ForegroundColor Cyan
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host ""
Write-Host "Downloading Tweakivo Beta ($installType)..." -ForegroundColor Cyan
    ...
    Write-Host "Installing Tweakivo Beta..." -ForegroundColor Cyan
    ...
    Write-Host "Launching Tweakivo..." -ForegroundColor Cyan
            Start-Process -FilePath $appPath
        }
    } else {
        # Portable installation - extract to Desktop
        Write-Host ""
    Write-Host "Extracting Tweakivo Beta Portable to Desktop..." -ForegroundColor Cyan
        ...
        Write-Host ""
    Write-Host "Tweakivo Portable has been extracted to your Desktop." -ForegroundColor Cyan
