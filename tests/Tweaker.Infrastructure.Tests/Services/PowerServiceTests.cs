...
public class PowerServiceTests
{
    ...
    [Fact]
    public async Task TryApplySpecialSettingAsync_CorruptTweakivoPlan_WhenRepairFails_DeletesGhostAndAttemptsImport()
    {
        // Arrange — ghost plan exists with the stable branded GUID but a corrupt name
        var tweakivoGuid = PowerPlanDefinitions.TweakivoPowerPlanGuid;
        ...
        _powerPlanComboBoxService
            ...
            .ReturnsAsync(new PowerPlanResolutionResult
            {
                Success = true,
                Guid = tweakivoGuid,
                DisplayName = PowerPlanDefinitions.TweakivoPowerPlanName
            });
        ...
        _powerSchemeOperations.Verify(
            s => s.DeleteScheme(Guid.Parse(tweakivoGuid)),
            Times.AtLeastOnce);

        // Should log that repair failed before cleanup
        _logService.Verify(
            l => l.Log(LogLevel.Warning, It.Is<string>(s => s.Contains("Failed to repair corrupt Tweakivo plan"))),
            Times.AtLeastOnce);

        // Name should be set on the actual GUID powercfg assigned, not the requested one
        _powerSchemeOperations.Verify(
            s => s.WriteFriendlyName(Guid.Parse(assignedGuid), PowerPlanDefinitions.TweakivoPowerPlanName),
            Times.Once);
    ...
    [Fact]
    public async Task TryApplySpecialSettingAsync_CorruptTweakivoPlan_WhenRepairSucceeds_DoesNotDelete()
    {
        var tweakivoGuid = PowerPlanDefinitions.TweakivoPowerPlanGuid;
        var setting = MakeSetting(SettingIds.PowerPlanSelection);
        ...
        _powerSchemeOperations
            .Setup(s => s.WriteFriendlyName(Guid.Parse(tweakivoGuid), PowerPlanDefinitions.TweakivoPowerPlanName))
            .Returns(PowerProf.ERROR_SUCCESS);
        _powerSchemeOperations
            .Setup(s => s.SetActiveScheme(Guid.Parse(tweakivoGuid)))
            .Returns(PowerProf.ERROR_SUCCESS);
        ...
        result.Should().BeTrue();
        _powerSchemeOperations.Verify(
            s => s.WriteFriendlyName(Guid.Parse(tweakivoGuid), PowerPlanDefinitions.TweakivoPowerPlanName),
            Times.Once);
    ...
    public async Task ImportPowerPlanAsync_LegacyNameWithCanonicalGuid_UsesBrandedImportAndCanonicalName()
    {
        ...
        _powerSchemeOperations
            .Setup(s => s.DeleteScheme(Guid.Parse(tweakivoGuid)))
            .Returns(PowerProf.ERROR_SUCCESS);
        _processExecutor
            ...
            {
                ExitCode = 0,
                StandardOutput = $"Power Scheme GUID: {tweakivoGuid}  (Ultimate Performance)"
            });
        _powerSchemeOperations
            .SetupSequence(s => s.WriteFriendlyName(Guid.Parse(tweakivoGuid), PowerPlanDefinitions.TweakivoPowerPlanName))
            .Returns(PowerProf.ERROR_FILE_NOT_FOUND)
            .Returns(PowerProf.ERROR_SUCCESS);
        _powerSchemeOperations
            .Setup(s => s.WriteDescription(Guid.Parse(tweakivoGuid), It.IsAny<string>()))
            .Returns(PowerProf.ERROR_SUCCESS);
        ...
        result.ImportedGuid.Should().Be(tweakivoGuid);
        _processExecutor.Verify(
            p => p.ExecuteAsync(
                "powercfg",
                It.Is<string>(args => args.Contains(tweakivoGuid, StringComparison.OrdinalIgnoreCase)),
                default),
            Times.Once);
        _powerSchemeOperations.Verify(
            s => s.WriteFriendlyName(Guid.Parse(tweakivoGuid), PowerPlanDefinitions.TweakivoPowerPlanName),
    ...
    [Fact]
    public async Task DiscoverSpecialSettingsAsync_CorruptTweakivoPlanActive_RepairFailsThenDeletesAfterSwitchingToBalanced()
    {
    ...
    public async Task DiscoverSpecialSettingsAsync_LegacyTweakerPlanActive_RenamesInPlaceAndDoesNotDelete()
    {
        ...
        result[SettingIds.PowerPlanSelection]["ActivePowerPlan"].Should().Be(PowerPlanDefinitions.TweakivoPowerPlanName);
        result[SettingIds.PowerPlanSelection]["ActivePowerPlanGuid"].Should().Be(tweakivoGuid);
    }
    ...
    public async Task TryApplySpecialSettingAsync_ConfigImportSuppliesPowerValues_SkipsRecommendedReapply()
    {
        ...
        _powerSchemeOperations.Verify(
            s => s.WriteFriendlyName(
                Guid.Parse(PowerPlanDefinitions.TweakivoPowerPlanGuid),
                PowerPlanDefinitions.TweakivoPowerPlanName),
            Times.Once);
        _powerSchemeOperations.Verify(s => s.DeleteScheme(It.IsAny<Guid>()), Times.Never);
    }
